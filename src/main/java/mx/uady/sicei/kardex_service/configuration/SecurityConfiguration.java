...
public class SecurityConfiguration {
    ...
    SecurityFilterChain configureSecurityFilterChain(HttpSecurity httpSecurity) throws Exception {
        return httpSecurity
                .authorizeHttpRequests(requests -> {
                    ...
                    requests.requestMatchers("/docs", "/webjars/**", "/swagger-ui/**", "/v3/api-docs/**").permitAll();
    ...
    private void secureAPI(
        ...
        requests.requestMatchers(HttpMethod.GET, path).hasAuthority(String.format("PERMISSION:%s:read", entity));
        requests.requestMatchers(HttpMethod.POST, path).hasAuthority(String.format("PERMISSION:%s:create", entity));
        requests.requestMatchers(HttpMethod.PUT, path).hasAuthority(String.format("PERMISSION:%s:update", entity));
        requests.requestMatchers(HttpMethod.PATCH, path).hasAuthority(String.format("PERMISSION:%s:update", entity));
        requests.requestMatchers(HttpMethod.DELETE, path).hasAuthority(String.format("PERMISSION:%s:delete", entity));
    }
}
